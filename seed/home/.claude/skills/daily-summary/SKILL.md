...
Fetch each user's events since the start of today and keep the merged MRs
(`accepted MergeRequest`) plus anything still open:

```bash
glab api "users/34/events?after=<yesterday>&per_page=100" \
  | jq -r '.[] | "\(.created_at) \(.action_name) \(.target_type) \(.target_title)"'
...
- **Per repo** — one section per repo touched (`hizi-engine-sdk`, `hizi-engine`,
  `hizi-engine-creator`, `hearth`, …), each listing the merged MR titles.
  Keep the real MR/commit title text (e.g. `fix(fixed-odds): …`).
